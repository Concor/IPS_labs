...
#pragma omp parallel for
		for (int i = k + 1; i < rows; ++i) {
			koef = -matrix[i][k] / matrix[k][k];
			double* matrix_i = matrix[i], *matrix_k = matrix[k];
			for (int j = k; j <= rows; ++j) {
				matrix_i[j] += koef * matrix_k[j];
			}
		}
	}
	chrono::high_resolution_clock::time_point t2 = chrono::high_resolution_clock::now();
...
//	}
//
//	printf("Serial Small Solution (elapsed time = %.8f seconds):\n", elapsed_time.count());
//	for (i = 0; i < test_matrix_lines; ++i) {
//		printf("x(%d) = %lf\n", i, result[i]);
//	}
//	delete[] result;
//#elif MATRIX_IN_USE == 2
	...
	printf("Parallel Big Solution (elapsed time = %.8f seconds):\n", elapsed_time.count());
	...
	DeInitMatrix(matrix);
	delete[] matrix;
//#elif MATRIX_IN_USE == 3
	//double** matrix = new double* [MATRIX_SIZE];
	//InitMatrix(matrix);
	//double* result = new double[MATRIX_SIZE];

	//chrono::duration<double> elapsed_time = SerialGaussMethod(matrix, MATRIX_SIZE, result);

	//printf("Serial Big Solution (elapsed time = %.8f seconds):\n", elapsed_time.count());
	//for (i = 0; i < MATRIX_SIZE; ++i) {
	//	printf("x(%d) = %lf\n", i, result[i]);
	//}
...
//	test_matrix[3][0] = 3; test_matrix[3][1] = 8;  test_matrix[3][2] = 9;  test_matrix[3][3] = 2;  test_matrix[3][4] = 37;
//
//	chrono::duration<double> elapsed_time = ParallelGaussMethod(test_matrix, test_matrix_lines, result);
//
//	for (i = 0; i < test_matrix_lines; ++i) {
//		delete[]test_matrix[i];
//	}
//
//	printf("Parallel Small Solution (elapsed time = %.8f seconds):\n", elapsed_time.count());
//	for (i = 0; i < test_matrix_lines; ++i) {
//		printf("x(%d) = %lf\n", i, result[i]);
//	}
//	delete[] result;
